...
void main(void) 
{
    ...
    vec3 oricol = texture2D(uImage0, q).xyz;
    vec3 col;

    // start with the source texture and misalign the rays it a bit
    // TODO animate misalignment upon hit or similar event
    col.r = texture2D(uImage0,vec2(uv.x+0.003,-uv.y)).x;
    col.g = texture2D(uImage0,vec2(uv.x+0.000,-uv.y)).y;
    col.b = texture2D(uImage0,vec2(uv.x-0.003,-uv.y)).z;
