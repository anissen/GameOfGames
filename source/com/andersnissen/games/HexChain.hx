...
class HexChain extends GameState
{
    ...
    override function setup() :Void
    {
        ...
        var collectableDotMap :Array<Array<Bool>> = [ for (x in 0...5) [ for (y in 0...9) false ]];
        var collectableDotCount = 0;
        while (collectableDotCount < 20) {
            var x = FlxRandom.intRanged(0, 4);
        ...
        for (y in 0...9) {
            for (x in 0...5) {
                ...
                hex.drawPolygon(polys.copy(), color); //, { color: FlxColor.BLACK, thickness: 0.0 } );

                // flixel.tweens.FlxTween.tween(hex.scale, { x: -1 }, 2, { startDelay: (y + x) / 10 });
                hexSprites.add(hex);
            }
        }
    ...
    function hexTouched(hex :FlxSprite, point :flixel.util.FlxPoint)
    {
        ...
        if (chain.length == 1) {
            chainSprite.drawCircle(chain[0].getMidpoint().x, chain[0].getMidpoint().y, hexRadius / 2, FlxColor.YELLOW);
        }
        for (chainIndex in 1...chain.length) {
            chainSprite.drawLine(chain[chainIndex - 1].getMidpoint().x, chain[chainIndex - 1].getMidpoint().y, chain[chainIndex].getMidpoint().x, chain[chainIndex].getMidpoint().y, { color: FlxColor.YELLOW, thickness: hexRadius } );
        }
    }
}
